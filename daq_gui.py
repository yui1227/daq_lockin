...
class LockinApp:
    def __init__(self, root: tk.Tk):
        ...
        # --- 變數儲存 ---
        self.sim_mode = tk.BooleanVar(value=True) # 預設開啟模擬模式
        ...
        # 參數輸入區
        self.create_input(control_frame, "採樣率 (Hz):", "50000", 0, 0)
        self.create_input(control_frame, "擷取時間 (s):", "1.0", 0, 1)
        self.create_input(control_frame, "時間常數 (s):", "0.05", 0, 2)
        
        # 設備設定
        self.create_input(control_frame, "設備名稱:", "Dev1", 1, 0)
        self.create_input(control_frame, "參考通道:", "ai0", 1, 1)
        self.create_input(control_frame, "訊號通道 (逗號隔開):", "ai1, ai2", 1, 2)

        range_frame = ttk.LabelFrame(control_frame, text="訊號通道輸入範圍 (Signal Input Range)")
        ...
        self.chk_sim = ttk.Checkbutton(btn_frame, text="模擬模式 (無硬體)", variable=self.sim_mode)
        self.chk_sim.pack(side=tk.LEFT, padx=10)

        self.btn_run = ttk.Button(btn_frame, text="開始擷取並分析", command=self.run_acquisition)
        self.btn_run.pack(side=tk.LEFT, padx=10)

        self.btn_save = ttk.Button(btn_frame, text="儲存數據 (.csv)", command=self.save_data, state=tk.DISABLED)
    ...
    def run_acquisition(self):
        ...
        if self.sim_mode.get():
            data, n_samples = self.generate_mock_data(fs, duration, len(sig_chs))
        else:
            if not NIDAQ_AVAILABLE:
                messagebox.showerror("錯誤", "未安裝 nidaqmx 套件，無法執行硬體擷取。")
                self.lbl_status.config(text="錯誤", foreground="red")
                return
            data, n_samples = self.acquire_real_data(fs, duration, dev_name, ref_ch, sig_chs, selected_range)
            if data is None:
        ...
        for i, res in enumerate(results):
            ...
            # 振幅
            self.axs[1].plot(t, res["R"], c=c, label=res["name"])
            # 相位
            self.axs[2].plot(t, res["Theta"], c=c, label=res["name"])

        # 設定圖表標籤
        self.axs[0].set_title(f"原始訊號 (Ref Freq: {freq:.2f}Hz, Range: ±{selected_range}V)")
        ...
        self.lbl_status.config(text=f"完成 (Ref: {freq:.2f}Hz)", foreground="green")
    ...
    def save_data(self):
        ...
        try:
            ...
            with open(filename, 'w', newline='', encoding='utf-8') as f:
                ...
                for res in results:
                    name = res["name"]
                    header.extend([f"{name}_Raw(V)", f"{name}_R(V)", f"{name}_Theta(deg)"])
    ...
    def acquire_real_data(self, fs, duration, dev, ref_ch, sig_chs, vol_range):
        ...
        try:
            with nidaqmx.Task() as task:
                # 設定通道
                task.ai_channels.add_ai_voltage_chan(f"{dev}/{ref_ch}", min_val=-vol_range, max_val=vol_range)
                for ch in sig_chs:
                    task.ai_channels.add_ai_voltage_chan(f"{dev}/{ch}", min_val=-vol_range, max_val=vol_range)
