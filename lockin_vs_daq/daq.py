...
class DAQ(Instrument):
    ...
    def set_real_time_acquisition_params(self, **params):
        ...
        self.task = nidaqmx.Task()
        self.task.ai_channels.add_ai_voltage_chan(f"{self.DAQ_NAME}/{self.DAQ_CHANNEL}")
        # 配置採樣時序
    ...
    def acquire_real_time_data(self) -> float | np.ndarray:
        ...
        獲取即時資料
        """
        new_data = self.task.read(number_of_samples_per_channel=self.SAMPLES_PER_READ)
    ...
    def set_timed_acquisition_params(self, **params):
        ...
        self.MEASUREMENT_DURATION = params.get("MEASUREMENT_DURATION")     # 量測持續時間 (秒)

        # 計算需要採集的總樣本數
        self.number_of_samples = int(self.SAMPLE_RATE * self.MEASUREMENT_DURATION)
        self.task = nidaqmx.Task()
        self.task.timing.cfg_samp_clk_timing(
            rate=self.SAMPLE_RATE, 
    ...
    def acquire_timed_data(self) -> np.ndarray:
        ...
        # 等待數據採集完成
        print("\n開始量測... 請稍候...")
        self.task.wait_until_done(timeout=self.MEASUREMENT_DURATION + 2.0) # 給予額外的時間裕度
        
        # 讀取所有採集到的數據
        acquired_data = self.task.read(number_of_samples_per_channel=self.number_of_samples)
